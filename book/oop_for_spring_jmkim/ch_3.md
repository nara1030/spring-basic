...
이에 대한 답은 모두 객체 지향 설계 5원칙과 관련 있다. 첫 번째 질문에 대한 답은 **상위 클래스는 물려줄 특성이 풍성할수록 좋고, 인터페이스는 구현을 강제할 메서드의 개수가 적을수록 좋다**고 할 수 있다(∵ LSP). 그리고 두 번째 질문, 즉 인터페이스에 메서드가 적을수록 좋은 이유는 ISP에 따른 이유라고 할 수 있다. 자세한 내용은 다음 [링크](ch_5.md)에서 확인할 수 있다.
